...
import { ImageService } from '../../../core/services/image-service/image.service';
...
export class ProfileComponent {
  ...
  public constructor(private route: ActivatedRoute, private userService: UserService, private objectService: ObjectService,
    ...
    this.userService.getById(Number(this.route.snapshot.paramMap.get('id'))).subscribe(
      user => {this.user = user});
    ...
    if (input.files && input.files.length > 0) {
      try {
        ...
        if (base64Images.length > 0 && this.user) {
          this.user.photoName = base64Images[0];
        }
      } catch (error) {
